...
def ask_user_input():
    ...
    user_input = input(
        "Press Enter to continue, 's' to stop, or 'c' to continue without prompting: "
    )
...
class rrCache:
    ...
    def __init__(
        ...
        cache_cfg_fln = os_path.join(
            install_dir, "config", f"config_{self.__cspace}.json"
        )
        ...
        self.__input__cache_dir = os_path.join(
            install_dir, "input-cache", self.__cspace
        )
    ...
    def Load(
        ...
        if attrs != []:
            if not isinstance(attrs, list):
                self.logger.warning(
                    "'attrs' argument is not of type list, trying to convert..."
                )
        ...
        if not do_not_dwnl_cache:
            try:
                rrCache._check_or_download_cache_to_disk(
                    self.__cache_dir, self.__attributes_list, self.logger
                )
        ...
        for attr in attributes_list:
            ...
            if os_path.exists(full_filename):
                ...
                if rrCache.__cache[attr]["file"]["url"] == "":
                    ...
                    download(
                        rrCache.__cache[attr]["file"]["url"] + filename, full_filename
                    )
    ...
    def Build(self, interactive: bool = DEFAULTS["interactive"]) -> None:
        ...
        if os_path.isdir(self.__cache_dir):
            self.logger.warning(
                f"Cache directory {self.__cache_dir} already exists, data might be overwritten"
            )
        else:
        ...
        if os_path.isdir(self.__input__cache_dir):
            self.logger.warning(
                f"Input cache directory {self.__input__cache_dir} already exists, existing data can be used, remove the directory to download new data"
            )
        else:
        ...
        try:
            rrCache._gen_inchikey_cid(
                self.__input__cache_dir, self.__cache_dir, cid_strc, self.logger
            )
    ...
    def _gen_deprecatedCID_cid(
        ...
        f_deprecatedCID_cid = os_path.join(
        ...
        if os_path.exists(f_deprecatedCID_cid) and check_sha(
            f_deprecatedCID_cid, rrCache.__cache[attribute]
        ):
            ...
            rrCache._store_cache_to_file(
                deprecatedCID_cid, f_deprecatedCID_cid, logger=logger
            )
    ...
    def _gen_inchikey_cid(
        ...
        logger.debug(c_attr("bold") + attribute + c_attr("reset"))
        inchikey_cid = None
        f_inchikey_cid = os_path.join(
            outdir, rrCache.__cache[attribute]["file"]["name"]
        )

        # Do not checksum since it is a dictionary
        if os_path.exists(f_inchikey_cid) and check_sha(
            f_inchikey_cid, rrCache.__cache[attribute]
        ):
            logger.debug("   Cache file already exists")
        else:
    ...
    def _gen_cid_xref(
        ...
        if os_path.exists(f_cid_xref) and check_sha(
            f_cid_xref, rrCache.__cache[attribute]
        ):
            ...
            logger.debug(f"   dep_files: {dep_files}")
    ...
    def _gen_deprecatedRID_rid(
        ...
        logger.debug(c_attr("bold") + attribute + c_attr("reset"))
        deprecatedRID_rid = None
        f_deprecatedRID_rid = os_path.join(
            outdir, rrCache.__cache[attribute]["file"]["name"]
        )

        # Do not checksum since it is a dictionary
        if os_path.exists(f_deprecatedRID_rid) and check_sha(
            f_deprecatedRID_rid, rrCache.__cache[attribute]
        ):
            ...
            logger.debug("   Writing data to file...")
            rrCache._store_cache_to_file(
                deprecatedRID_rid, f_deprecatedRID_rid, logger=logger
            )
    ...
    def _gen_comp_xref_deprecatedCompID_compid(
        ...
        f_deprecatedCompID_compid = os_path.join(
            outdir, rrCache.__cache["deprecatedCompID_compid"]["file"]["name"]
        )

        # Do not checksum since it is a dictionary
        if (
            os_path.exists(f_comp_xref)
            and check_sha(f_comp_xref, rrCache.__cache["comp_xref"])
            and os_path.exists(f_deprecatedCompID_compid)
            and check_sha(
                f_deprecatedCompID_compid, rrCache.__cache["deprecatedCompID_compid"]
            )
        ):
            ...
            logger.debug("   Writing data to file...")
            rrCache._store_cache_to_file(
                deprecatedCompID_compid, f_deprecatedCompID_compid, logger=logger
            )
            # print_OK()
    ...
    def _check_or_load_cache(self):
        ...
        for attribute in self.__attributes_list:
            ...
            if self.get(attribute) is None:
                self.set(
                    attribute, self._load_json(os_path.join(self.__cache_dir, filename))
                )
                print_progress(self.logger)
            else:
    ...
    def _store_cache_to_file(data, filename, logger: Logger = getLogger(__name__)):
        ...
        if filename.endswith(".gz") or filename.endswith(".zip"):
            ...
            json_bytes = json_dumps(data, sort_keys=True, separators=(",", ":")).encode(
                "utf-8"
            )
    ...
    def _m_mnxm_strc(
        ...
        if chem_prop_path:
            ...
            with open(chem_prop_path, "rt", encoding="utf-8-sig") as f:
                tmp = {}
            ...
            logger.debug(
                "No chem_prop.tsv or deprecatedCID_cid provided, skipping structure parsing"
            )

        if comp_xref_path:
            # Add cross references from comp_xref.tsv
            comp_xref, _ = rrCache._m_mnxc_xref(comp_xref_path, logger=logger)
            for mnxc in comp_xref:
                if mnxc in cid_strc:
                    cid_strc[mnxc]["xref"] = comp_xref[mnxc]
                    logger.debug(
                        f"Added cross references for {mnxc}: {comp_xref[mnxc]}"
                    )
        else:
